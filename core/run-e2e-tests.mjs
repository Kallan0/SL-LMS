/**
 * E2E Test Runner for Sign Language LMS Backend
 * 
 * This script:
 * 1. Starts the backend server on port 5050
 * 2. Runs all E2E tests against it
 * 3. Cleans up when done
 * 
 * Run: node run-e2e-tests.mjs
 */
...
async function testErrorHandling() {
  ...
  await test("POST /auth/login - missing body", async () => {
    ...
    // Server returns 401 or 500 when body is incomplete
...
function startServer() {
  return new Promise((resolve, reject) => {
    // Set PORT env and start the compiled JS
    ...
    // Resolve after a short delay - the server should be ready
